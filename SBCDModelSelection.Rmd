  ...
  if (sum(data$Pres[siteInd]>0)>50){
    ...
    for (i in 1:length(profileList)){ # for profiles of covars with multiple depth layers
      ...
      for (m in 1:(length(dateMarks)-1)){
        ...
        # grid.arrange(plotList$Pres,grid.arrange(plotList[[profileName]],plotList[["ColorLegend"]],ncol=2,widths=c(10,1)),
        #              ncol=1,nrow=3,layout_matrix=rbind(1,2,2),top=paste('Cuvier at',sites[j]))
        #grid.arrange(plotList$Pres,plotList[[profileName]],plotList[["ColorLegend"]],ncol=12,nrow=3,layout_matrix=rbind(c(rep(1,11),NA),
                                                                                                                        #c(rep(2,11),NA),
                                                                                                                        #c(rep(2,11),3)))
      }
    }
...
```
SSH and Sal200 are collinear, removing SSH
