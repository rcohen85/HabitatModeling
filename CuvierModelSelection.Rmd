...
for (j in 1:length(sites)){
  ...
  if (sum(data$Pres[siteInd]>0)>50){
    ...
    # plotDF = data.frame(Pres = data$Pres[siteInd],
    #                     Date = as.Date(data$Date[siteInd],origin="1970-01-01"))
      ...
      for (m in 1:(length(dateMarks)-1)){
        ...
        plotList[['Pres']] = ggplot(plotDF,aes(x=Date) # plot presence in this year
          ...
          plotList[[profileName]] = ggplot(profileDF,aes(x=x,y=y,z=z) # plot profile for this year
            ...
            # )+scale_fill_discrete(rainbow(8)
            # )+scale_color_gradientn(
            # palette=my_colors,
            #       breaks=seq(0,315,45),
            #       label=direction_labeller(seq(0,315,45)),
            #       limits=c(0,359)
          )+scale_fill_manual(
            values=my_colors,
            guide=NULL
          ...
          grid.arrange(plotList$Pres,plotList[[profileName]],plotList[["ColorLegend"]],ncol=12,nrow=3,top=paste('Cuvier at',sites[j]),layout_matrix=rbind(c(rep(1,11),NA),
                                                                                                                        c(rep(2,11),NA),
                                                                                                                        c(rep(2,11),3)))
        ...
        plotList[['Pres']] = ggplot(plotDF,aes(x=Date) # plot presence in this year
          ...
          #plotList[["ColorLegend"]] = ggplot()
          
          grid.arrange(plotList$Pres,plotList[[profileName]],ncol=12,nrow=3,top=paste('Cuvier at',sites[j]),layout_matrix=rbind(c(rep(1,12)),
                                                                                                c(rep(2,12)),
                                                                                                c(rep(2,12))))
        }
      }
