...
for (i in 1:length(smoothVarList)){
  ...
  eval(parse(text=paste('smoothMod=geeglm(Pres~mSpline(data$',smoothVarList[i],',knots=quantile(data$',smoothVarList[i],',prob=c(0.275,0.5,0.725)),Boundary.knots=c(min(data$',smoothVarList[i],'),max(data$',smoothVarList[i],'))),family=poisson,data=data,id=GroupID,corstr=\"ar1\")',sep="")))
