...
for (i in 1:length(smoothVarList)){
  ...
  eval(parse(text=paste('smoothMod=geeglm(Pres~mSpline(',smoothVarList[i],',knots=quantile(data$',smoothVarList[i],',prob=c(0.275, 0.5, 0.725)),Boundary.knots=c(min(data$',smoothVarList[i],'),max(data$',smoothVarList[i],')),family=poisson,
             data=data,
             id=GroupID,
            corstr=\"ar1\")',sep="")))
...
for (i in 1:length(smoothVarList)){
 ...
 thisForm = formula(paste('Pres~',paste(smoothNameList,collapse="+"),'+Slope+Aspect',sep=""))
...
thisForm = formula(paste('Pres~',paste(smoothNameList,collapse="+"),'+Slope+Aspect+S_VelAsp700Lag28:Aspect',sep=""))
